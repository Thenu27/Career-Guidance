...
const OLevelAdd = () => {
    ...
    const sendToDataToBE = async () => {
        try {
            const response = await axios.post(`${import.meta.env.VITE_APP_URL}/api/ordinarylevel/add`, {
            ...
            <div className='ol-add-container'>
                <div className='ol-add-inner-container'>
                    <div className='ol-input-container'>
                        ...
                        <input type='text' value={NewSubject} onChange={(event)=>{handleOnChange(event)}} className='ol-input' />
                    </div>
